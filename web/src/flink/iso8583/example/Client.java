...
import flink.iso8583.impl.SimpleTraceGenerator;
import flink.iso8583.parse.ConfigParser;
...
public class Client extends Thread {
	...
	/** This map holds the messages that are still awaiting response. Keys are traces, values are IsoMessages.*/
			...
			while (sock != null && sock.isConnected() && !isInterrupted()) {
				...
				if (sock.getInputStream().read(buf) == size) {
					try {
						...
						log.debug("Read response " + resp.getField(11) + " conf " + resp.getField(38)
							+ ": " + new String(buf));
