...
public class Paginate extends SimpleTagSupport {
	...
	private void appendPageNavigationHtml(StringBuffer html) {
		...
		showSingleSummary = (String)request.getSession().getServletContext().getInitParameter("showSingleSummary");
		...
		if (StringUtils.isEmpty(navigateStyle)) {
			navigateStyle = (String)request.getSession().getServletContext().getInitParameter("htmlNavigator");
		}
