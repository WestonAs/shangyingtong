 ...
 *
 * @description: 商户新增审核流程适配器(非管理机构添加时，由其管理的分支机构来审核)
 *
 ...
 */
@Repository
public class MerchCardAdapter implements WorkflowAdapter {
	...
	private static Logger logger = Logger.getLogger(MerchCardAdapter.class);

	public void flowEnd(String refid, String param, String userId)
			throws BizException {
		logger.debug("非管理机构新增商户审核通过的相关处理。");
