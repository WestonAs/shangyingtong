 ...
 */
@Repository
public class Msg2022Adapter implements MsgAdapter {
	@Autowired
	...
	private void dealForFailure(SaleCardReg saleCardReg) throws BizException {
		...
		for(String cardId : cardIdList){
			...
			if (cardInfo.isPreselledStatus()) {
				cardInfo.setCardStatus(CardState.FORSALE.getValue());
	...
	private String getNote(Long id) throws BizException {
		Waitsinfo waitsinfo = this.waitsinfoDAO.findWaitsinfo(MsgType.PRE_SELL_ACTIVATE, id);
	...
	private IcCardExtroInfo buildIcCardExtroInfo(CardInfo cardInfo) throws BizException {
		...
		Assert.isNull(icCardExtroInfoDAO.findByPk(cardInfo.getCardId()), "IC卡附加信息表中已经存在卡号[" + cardInfo.getCardId() + "]的记录");
	...
	private void dealCardRisk(SaleCardReg saleCardReg, String cardSubClassStr) throws BizException {
		...
		// 把扣掉的售卡配额补回来。 发卡机构自己不做限制
		if (!StringUtils.equals(saleCardReg.getBranchCode(), saleCardReg.getCardBranch())) { // 预售卡操作机构不是发卡机构
