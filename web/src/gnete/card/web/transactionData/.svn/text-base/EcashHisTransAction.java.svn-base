 ...
 *
 */
public class EcashHisTransAction extends BaseAction {

	private static final String IS_GENERATING_ECASH_HIS_TRANS_EXCEL = "IS_GENERATING_ECASH_HIS_TRANS_EXCEL";
	@Autowired
...
//		this.transTypeList = isUserOfLimitedTransQuery ? TransType.getLimitedTransQueryType() : TransType.ALL.values(); // 加载交易类型
...
//			params.put("transType",trans.getTransType());
		...
		params.put("transType","32");//只能查电子现金消费
...
//			DbContextHolder.setDbType(DbType.TRANS_DB);
	...
	public void generate() throws Exception {
		ActionContext.getContext().getSession().put(IS_GENERATING_ECASH_HIS_TRANS_EXCEL,Boolean.TRUE);
...
//				params.put("transType",trans.getTransType());
			...
			if (isCardOrCardDeptRoleLogined()){
				if (trans == null || StringUtils.isBlank(trans.getCardIssuer())) {
					branchList = this.branchInfoDAO.findTreeBranchList(getLoginBranchCode());;
				}
			} else if (isMerchantRoleLogined()){
				params.put("merNo", this.getSessionUser().getMerchantNo());
			} else if (isTerminalRoleLogined()){
			...
			if(StringUtils.isNotBlank(settStartDate) && StringUtils.isNotBlank(settEndDate)){ //如果清算日期不为空，检查跨度是否超过30天
				if(DateUtil.chkDateFormat(settStartDate) && DateUtil.chkDateFormat(settEndDate)){ //检查清算日期格式
					...
					if (StringUtils.isEmpty(this.getStartCardId()) && StringUtils.isEmpty(this.getEndCardId())) {
						if(diffDays > 31){
							throw new BizException("卡号为空时，清算日期的查询间隔不得超过31天");
						}
					}
				}
			}
			...
			this.generateFileService.generateEcashHisTransExcel(response, params);
		}finally{
			ActionContext.getContext().getSession().remove(IS_GENERATING_ECASH_HIS_TRANS_EXCEL);
		}
	}
	...
	public void ajaxIsGeneratingExcel() throws Exception {
		...
		if (Boolean.TRUE.equals(ActionContext.getContext().getSession().get(IS_GENERATING_ECASH_HIS_TRANS_EXCEL))) {
