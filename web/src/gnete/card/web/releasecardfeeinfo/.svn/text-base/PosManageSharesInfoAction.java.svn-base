 ...
 * 维护方分润查询
 */
public class PosManageSharesInfoAction extends BaseAction{
	...
	private List<BranchInfo> posManagevList;
	private boolean showTerminalMaintain = false;
	...
	public String execute() throws Exception {
		...
		if ( releaseCardFeeDTotal != null) {
			params.put("posManageName", MatchMode.ANYWHERE.toMatchString(releaseCardFeeDTotal.getPosManageName()));
		...
		// 机具维护方
		else if (RoleType.TERMINAL_MAINTAIN.getValue().equals(this.getLoginRoleType())){
			showTerminalMaintain = true;
			posManagevList = this.getMyCardBranch();
			params.put("proxyId", posManagevList.get(0).getBranchCode());
		} else {
			throw new BizException("没有权限查询维护方分润！");
		}

 		this.page = this.releaseCardFeeDTotalDAO.findPosManageSharesInfo(params, this
	...
	public List<BranchInfo> getPosManagevList() {
		return posManagevList;
	}

	public void setPosManagevList(List<BranchInfo> posManagevList) {
		this.posManagevList = posManagevList;
	}

	public boolean isShowTerminalMaintain() {
		return showTerminalMaintain;
	}

	public void setShowTerminalMaintain(boolean showTerminalMaintain) {
		this.showTerminalMaintain = showTerminalMaintain;
	}
