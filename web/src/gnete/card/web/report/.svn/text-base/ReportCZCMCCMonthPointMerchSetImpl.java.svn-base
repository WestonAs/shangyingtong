...
@Service("reportCZCMCCMonthPointMerchSet")
public class ReportCZCMCCMonthPointMerchSetImpl extends AbstractCardReportLoadImpl{
	 ...
	 *  优惠积分项目商家结算一览表
	 */
	private final String MONTH_POINT_MERCH_SET_FILE = "/chaozhouCMCC/monthPointMerchSet.raq";

	private final String REPORT_ERROR_MSG = "没有权限查看潮州移动的优惠积分项目商家结算一览表!";
	...
	protected void processUserLoad(HttpServletRequest request, UserInfo userInfo) {
		...
		if (StringUtils.equals(loginRoleType, RoleType.CENTER.getValue())
			...
			reportFile = MONTH_POINT_MERCH_SET_FILE;
		}
