 ...
 * 
  * @Project: Card
  * @File: MerchProxyShareReportImpl.java
  * @See:
  * @description：
  *   <li>针对商户代理所看到的"商户代理分润报表"</li> 
  * @author: aps-zbw
  * @modified:
  ...
  * @version:  V1.0
 */
@Service("cardProxyShareReport")
public class CardProxyShareReportImpl extends AbstractCardReportLoadImpl{
	@Autowired
	private BranchInfoDAO branchInfoDAO;

	//private final String REPORT_CARD_FILE = "/fkjgFrMonthDs.raq";

	private final String REPORT_PROXY_FILE = "/cardProxy/shdlFrMonthDs.raq";
	...
	protected void processUserLoad(HttpServletRequest request, UserInfo userInfo) {
		...
		if (StringUtils.equals(roleType, RoleType.CENTER.getValue())
			...
			request.setAttribute("shdlList", shdlList);
	...
	private String[] getTopLevelQueryParams(String roleType,String reportType,String[] params) throws Exception{
		 ...
		 */
		if (StringUtils.equals(reportType, ShfrReportType.MERCHANT.getValue())) {
