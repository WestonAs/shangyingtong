 ...
 *
 */
public class FreezeRegBatAction extends BaseAction {
	...
	public String execute() throws Exception {
		...
		if (freezeReg != null) {
			...
			params.put("branchCode", freezeReg.getBranchCode());
			params.put("branchName", MatchMode.ANYWHERE.toMatchString(freezeReg.getBranchName()));
		}

		this.page = this.freezeRegDAO.findFreezeBat(params, this.getPageNumber(), this.getPageSize());
	...
	public String detail() throws Exception {
		...
		this.log("查询冻结批次[" + this.freezeReg.getFreezeId() + "]明细信息成功",
				UserLogType.SEARCH);
		return DETAIL;
	}
	...
	public String add() throws Exception {			
		...
		//保存数据
		this.freezeRegService.addFreezeRegBat(freezeReg, this.getSessionUserCode());
		
		String msg = "新增卡账户冻结批次["+this.freezeReg.getFreezeId()+"]成功！";
		this.addActionMessage("/freezeBat/list.do", msg);
	...
	public void checkStartCardId() throws Exception {
		String startCard = this.freezeReg.getStartCard();
	...
	public void checkCardId() throws Exception {
		...
		try {
			...
			if (StringUtils.isNotEmpty(cardCount)) {
				...
				Assert.notTrue(count>1000 , "不能冻结超过1000张卡。");
			}
