IcEcashDeposit ={
	...
	startTrans: function(touchType){
		...
		var newDataLength = IcEcashDeposit.leftPad(length, 4, '0');
			...
			$.post(CONTEXT_PATH + '/pages/icEcashDeposit/searchCardId.do', 
				...
				function(data){
					...
					if(data.result) {
						...
						// 加载返利规则
						$jload('idRebateRulePage', '/pages/icEcashDeposit/findRebateRule.do', 
							{'formMap.cardId':cardId, 'formMap.cardCustomerId':data.cardCustomerId, 'callId':callId()});
					} else {
						showMsg(data.msg);
						return false;
					}
	...
	getTransARQC: function(touchType, authAmt) {
		...
		var input = a + IcEcashDeposit.leftPad(inputLast.length, 4, '0') + inputLast;
	 ...
	 */
	 getTransConfirm: function(touchType, authAmt){
	 	...
	 	var suffix = IcEcashDeposit.leftPad(CDOL1Data.length, 4, '0') + CDOL1Data + IcEcashDeposit.leftPad(CDOL2.length, 4, '0') + CDOL2;
	 	var suff = ensType + gnType + authAmt + otherAmt + authRespCode + suffix;
	 	
	 	var input = a + IcEcashDeposit.leftPad(suff.length, 4, '0') + suff;
	...
	runWriteScript: function(touchType, scriptData){
		...
		var dataLen = IcEcashDeposit.leftPad(scriptData.length, 4, '0');
	...
	writeCard: function(touchType, arpc, authAmt, scriptData){
		...
		if(!IcEcashDeposit.getTransConfirm(touchType, authAmt)){
			return "-1";
		}
		// 1021运行写卡脚本
		return IcEcashDeposit.runWriteScript(touchType, scriptData);
	},
	
	// 读取卡片信息
	readCardInfo: function(){
		IcEcashDeposit.clearAllInput();
		var touchType = FormUtils.getRadioedValue('touchType');
		// 1001 上电
		if(!IcEcashDeposit.powerOn(touchType)){
			return false;
		}
		// 1003 选择金融环境
		if(!IcEcashDeposit.onSelect(touchType)) {
			return false;
		}
		// 1004 开始交易
		if(!IcEcashDeposit.startTrans(touchType)) {
			return false;
		}
		//1008 获取卡片信息
		if(!IcEcashDeposit.getCardInfo(touchType)) {
			return false;
		}
		// 1011 取得卡片余额
		if(!IcEcashDeposit.getCardBanlance(touchType)) {
			return false;
		}
	},
	...
	checkForm: function(){
		...
		if(!IcEcashDeposit.checkEcashDeposit()) {
			return false;
		}
		...
		if(signatureReg == 'true'){
			if(!IcEcashDeposit.CheckUSBKey()){
				return false;
			}
		}
		
		IcEcashDeposit.loadDeal();
		
		var balance = $('#id_depositAmt').val();
		// 充值金额
		var newBal = new Array("","");
		var newBal = (new String(balance * 100)).split(".");
		//alert('newBal:' + newBal[0]);
		var authAmt = IcEcashDeposit.leftPad(newBal[0], 12, '0'); //授权金额即充值金额
		
		// 1017 获得交易密文ARQC
		if(!IcEcashDeposit.getTransARQC(touchType, authAmt)) {
			return false;
		}
		
		// 1012 获取终端验证结果
		if(!IcEcashDeposit.getTerminalCheck(touchType)) {
			return false;
		}
		...
		$.post(CONTEXT_PATH + '/pages/icEcashDeposit/add.do', 
				jsonObj, 
				function(data){
					// 获取信息成功的话
					if(data.result) {
						...
						var depositBatchId = data.depositBatchId;
						//alert("arpc:" + arpc);
						
						var revesalURL = '/pages/icEcashDeposit/reversal.do?icEcashReversal.amt=' + authAmt 
									+ '&icEcashReversal.cardId=' + cardId
									+ '&icEcashReversal.depositBatchId=' + depositBatchId;
						
						// 1020 进行发卡行认证
						var cardAuth = IcEcashDeposit.getCardBrancAuth(touchType, arpc);
						if(cardAuth == "-1"){
							gotoUrl(revesalURL);
							return false;
						}
						...
						if(chkRespCode == '00' && writeRespCode == '00'){
							// 写卡 
							writeRuslt = IcEcashDeposit.writeCard(touchType, arpc, authAmt, scriptData);
						}
						...
						// 1002 下电
						var power = IcEcashDeposit.powerOff(touchType);
						if(power == "-1"){
						...
						$('#id_reversal_depositBatchId').val(depositBatchId);
						...
						if(!isEmpty(writeRuslt)) {
							...
							// 写卡成功的时候发通知
								...
								gotoUrl('/pages/icEcashDeposit/notice.do?icEcashReversal.depositBatchId=' + depositBatchId);
						...
						//alert(data.msg);
						gotoUrl('/pages/icEcashDeposit/noticeError.do?errorMsg=' + data.msg);
					}
				}, 
			'json');
	},
	...
	checkEcashDeposit: function(){
		var depositAmt = $('#id_depositAmt').val();
		...
		if((parseFloat(lastBalance) + parseFloat(depositAmt)) > parseFloat(balanceLimit)){
			showMsg("“充值金额+余额”不能大于卡余额上限");
			return false;
		}
		return true;
	},
	...
	CheckUSBKey: function() {
	    ...
	    try{
			...
			if (isOnline == 0) {
				if(IcEcashDeposit.FTDoSign()){ //调用FT的签名函数
					return true;
				} else {
					return false;
				}
	...
	FTDoSign: function() {
		...
		if (SetCertResultRet == 0) {
			...
			$('#id_serialNo').val(serialNumber);
	   		...
	   		var depositAmt = $('#id_depositAmt').val(); //充值金额
			
			// 取得要验签的内容(卡号+ 充值金额)
	        var signResultRet = document.all.FTUSBKEYCTRL.SignResult(cardId +  fix(depositAmt) + randomNum);
			if (signResultRet == 0) {
...
function calRealAmt(){
	...
	var depositAmt = $('#id_depositAmt').val();
	if(depositAmt == undefined || depositAmt=="") return;

	$('#depositBtn').attr('disabled','disabled');
	$('#rebateAmt').val("");
	$.post(CONTEXT_PATH + '/pages/icEcashDeposit/calRealAmt.do', 
		{'icEcashDepositReg.rebateId':rebateId, 'icEcashDepositReg.depositAmt':depositAmt, 
			'formMap.cardId':$("#id_cardId").val(), 'callId':callId() }, 
		function(data){
