IcParaMgr ={
	...
	startTrans: function(touchType){
		...
		var newDataLength = IcParaMgr.leftPad(length, 4, '0');
	 ...
	 *  1025
	 *  获得卡片电子现金参数。
	 * 输入参数：1025 + I_DataLen + I_BalUpLimitFlag + I_SingleLimitFlag + I_ResetValveFlag
     * 		I_DataLen 数据长度此处为0003，四位
     * 		I_BalUpLimitFlag 电子现金余额上限标识。1 获得， 0 不获得；1位。
     * 		I_SingleLimitFlag 电子现金单笔消费限额标识。 1 获得， 0 不获得；1位。
	 *		I_ResetValveFlag 电子现金重置阈值标识。 1 获得， 0 不获得；1位。
	 * 
	 * 输出参数：10250038+ O_BalUpLimit + & + O_SingleLimit + & + O_ResetValve
	 *		O_BalUpLimit 电子现金余额上限 12 位
	 * 		O_SingleLimit 电子现金单笔消费限额 .标识 12 位
	 *		O_ResetValve 电子现金重置阈值 12 位
	 */
	getICParameter: function(touchType) {
		var a = this.inputPrex + touchType + "02B10250003111";
		//showMsg('1025 input:' + a);
		var lState = HiddenCOM.PBOCCommand(a);  // 10250038 + O_BalUpLimit + & + O_SingleLimit + & + O_ResetValve
		
		var str2 = lState.substr(4,4);		
		if(str2 == "9999") {
			showMsg('获得卡片电子现金参数失败！' + lState);
			return false;
		}
		// 处理成功
		else {
			//showMsg('1025 output:' + lState);
			var info = lState.split("&");//-- (10250038+电子现金余额上限) + 电子现金单笔消费限额 + 电子现金重置阈值

			var O_BalUpLimit = lState.substr(8,12);
			var O_SingleLimit = info[1];
			
			$('#id_old_balanceLimit').val(fix(new Number(O_BalUpLimit)/100));
			$('#id_old_amountLimit').val(fix(new Number(O_SingleLimit)/100));
		}
		return true;
	},
	...
	getCardInfo: function(touchType){
		...
		// 处理成功
			...
			$.post(CONTEXT_PATH + '/pages/icParaMgr/searchCardId.do', 
				{'cardId':cardId}, 
				function(data){
					// 获取信息成功的话
					if(data.result) {
						...
						$('#id_autoDepositFlag').val(data.autoDepositFlag);
						$('#id_autoDepositFlagName').val(data.autoDepositFlagName);
						$('#id_autoDepositAmt').val(fix(data.autoDepositAmt));
	...
	getTransARQC: function(touchType, authAmt) {
		...
		var input = a + IcParaMgr.leftPad(inputLast.length, 4, '0') + inputLast;
	 ...
	 */
	 getTransConfirm: function(touchType, authAmt){
	 	...
	 	var suffix = IcParaMgr.leftPad(CDOL1Data.length, 4, '0') + CDOL1Data + IcParaMgr.leftPad(CDOL2.length, 4, '0') + CDOL2;
	 	var suff = ensType + gnType + authAmt + otherAmt + authRespCode + suffix;
	 	
	 	var input = a + IcParaMgr.leftPad(suff.length, 4, '0') + suff;
	...
	runWriteScript: function(touchType, scriptData){
		...
		var dataLen = IcParaMgr.leftPad(scriptData.length, 4, '0');
	...
	writeCard: function(touchType, arpc, authAmt, scriptData){
		...
		if(!IcParaMgr.getTransConfirm(touchType, authAmt)){
			return "-1";
		}
		// 1021运行写卡脚本
		return IcParaMgr.runWriteScript(touchType, scriptData);
	},
	
	// 读取卡片信息
	readCardInfo: function(){
		IcParaMgr.clearAllInput();
		var touchType = FormUtils.getRadioedValue('touchType');
		// 1001 上电
		if(!IcParaMgr.powerOn(touchType)){
			return false;
		}
		// 1003 选择金融环境
		if(!IcParaMgr.onSelect(touchType)) {
			return false;
		}
		// 1004 开始交易
		if(!IcParaMgr.startTrans(touchType)) {
			return false;
		}
		//1025 获取卡片参数
		if(!IcParaMgr.getICParameter(touchType)) {
			return false;
		}
		//1008 获取卡片信息
		if(!IcParaMgr.getCardInfo(touchType)) {
			return false;
		}
		// 1011 取得卡片余额
		if(!IcParaMgr.getCardBanlance(touchType)) {
			return false;
		}
	},
	...
	checkForm: function(){
		...
		if(!IcParaMgr.checkEcashDeposit()) {
			return false;
		}
		...
		var authAmt = IcParaMgr.leftPad(newBal[0], 12, '0'); //授权金额
		
		// 1017 获得交易密文ARQC
		if(!IcParaMgr.getTransARQC(touchType, authAmt)) {
			return false;
		}
		
		// 1012 获取终端验证结果
		if(!IcParaMgr.getTerminalCheck(touchType)) {
			return false;
		}
		...
		$.post(CONTEXT_PATH + '/pages/icParaMgr/add.do', 
				jsonObj, 
				function(data){
					// 获取信息成功的话
					if(data.result) {
						...
						// 1020 进行发卡行认证
						var cardAuth = IcParaMgr.getCardBrancAuth(touchType, arpc);
						if(cardAuth == "-1"){
							alert("进行发卡行认证(1020)失败");
							return false;
						}
						...
						if(chkRespCode == '00' && writeRespCode == '00'){
							// 写卡 
							writeRuslt = IcParaMgr.writeCard(touchType, arpc, authAmt, scriptData);
						}
						...
						// 1002 下电
						var power = IcParaMgr.powerOff(touchType);
						if(power == "-1"){
							alert("写卡后，下电失败");
							return false;
						}
						...
						if(!isEmpty(writeRuslt)) {
							...
							// 写卡成功的时候发通知
								...
								gotoUrl('/pages/icParaMgr/notice.do?icCardParaModifyReg.id=' + id);
							}
						}
						return true;
					} else {
						$('#id_error_msg').val(data.msg);
						gotoUrl('/pages/icParaMgr/noticeError.do?errorMsg=' + data.msg);
					}
				}, 
			'json');
	},
	...
	checkEcashDeposit: function(){
		...
		var cardSubClass = $('#id_cardSubClass').val();
		...
		/* 
		 * 由于以前某些卡没有该字段，所以不需要限制该字段必填
		var autoDepositFlag = $('#id_autoDepositFlag').val();
		if(isEmpty(autoDepositFlag)){
			showMsg("是否自动圈存不能为空！");
			return false;
		}
		*/
		var oldBalanceLimit = $('#id_old_balanceLimit').val();
		if(isEmpty(oldBalanceLimit)){
			showMsg("修改前电子现金余额上限不能为空！");
			return false;
		}
		var oldAmountLimit = $('#id_old_amountLimit').val();
		if(isEmpty(oldAmountLimit)) {
			showMsg("修改前单笔交易限额不能为空！");
			return false;
		}
		...
		if(isEmpty(balanceLimit)){
			showMsg("修改后电子现金余额上限不能为空！");
			return false;
		}
		var amountLimit = $('#id_amountLimit').val();
		if(isEmpty(amountLimit)){
			showMsg("修改后修改后单笔交易限额不能为空！");
			return false;
		}
		return true;
	},
	...
	CheckUSBKey: function() {
	    ...
	    try{
			...
			if (isOnline == 0) {
				if(IcParaMgr.FTDoSign()){ //调用FT的签名函数
					return true;
				} else {
					return false;
				}
	...
	FTDoSign: function() {
		...
		if (SetCertResultRet == 0) {
			...
			$('#id_serialNo').val(serialNumber);
	   		...
	   		var amountLimit = $('#id_amountLimit').val(); //交易限额
			
			// 取得要验签的内容(卡号+ 充值金额)
	        var signResultRet = document.all.FTUSBKEYCTRL.SignResult(cardId +  fix(amountLimit) + randomNum);
			if (signResultRet == 0) {
