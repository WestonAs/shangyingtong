		$(function(){
			$('#idMembClass').blur(function(){
				$('#idMembClass_field').html('请输入8位数字会员类型');
				var value = $(this).val();
				...
				$.post(CONTEXT_PATH + '/vipCard/isExistMembClass.do', {'membClassDef.membClass':value}, function(json){
					if (json.isExist){
						$('#idMembClass_field').html('该会员类型已存在，请更换').addClass('error_tipinfo').show();
		...
		function clearMembClassError(){
			$('#idMembClass_field').removeClass('error_tipinfo').html('请输入8位数字会员类型');
