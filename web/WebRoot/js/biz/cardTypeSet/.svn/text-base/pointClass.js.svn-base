PointClass = {
	jinstTypeChange: function (id_issName,id_issId,id_issType) {
		$('#'+id_issName).removeMulitselector();
		var jinstType = $('#'+id_issType).val();
		...
		if(jinstType=='0'){
		...
		}else if(jinstType=='1'||jinstType=='2'){
			...
			$('#jinst_td2').show();
			if(jinstType=='1'){
				Selector.selectMerch('jinstName', 'Id_jinstId', true);
			}
			if(jinstType=='2'){
				Selector.selectMerchGroup('jinstName', 'Id_jinstId', true);
			}	
		}else{
			$('#jinst_td1').hide();
			$('#jinst_td2').hide();
	...
	ptUsageChange: function () { 
		...
		//浮动积分费率
		if(ptUsage=='6'){
			$('#id_ptDiscntRate').val('1');
			$('#ptDiscntRate_tr').hide();
		}else{
			$('#ptDiscntRate_tr').show();
			if($("#origPtDiscntRate").val()!=null && $("origPtDiscntRate").val()!="null"){
				$('#id_ptDiscntRate').val($("#origPtDiscntRate").val());
			}else{
				$('#id_ptDiscntRate').val('');
			}
		}
