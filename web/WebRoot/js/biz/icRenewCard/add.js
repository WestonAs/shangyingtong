IcRenewCard ={
	...
	startTrans: function(touchType){
		...
		var newDataLength = IcRenewCard.leftPad(length, 4, '0');
	...
	getTransARQC: function(touchType, authAmt) {
		...
		var input = a + IcRenewCard.leftPad(inputLast.length, 4, '0') + inputLast;
	 ...
	 */
	 getTransConfirm: function(touchType, authAmt){
	 	...
	 	var suffix = IcRenewCard.leftPad(CDOL1Data.length, 4, '0') + CDOL1Data + IcRenewCard.leftPad(CDOL2.length, 4, '0') + CDOL2;
	 	var suff = ensType + gnType + authAmt + otherAmt + authRespCode + suffix;
	 	
	 	var input = a + IcRenewCard.leftPad(suff.length, 4, '0') + suff;
	...
	runWriteScript: function(touchType, scriptData){
		...
		var dataLen = IcRenewCard.leftPad(scriptData.length, 4, '0');
	...
	writeCard: function(touchType, arpc, authAmt, scriptData){
		...
		if(!IcRenewCard.getTransConfirm(touchType, authAmt)){
			return "-1";
		}
		// 1021运行写卡脚本
		return IcRenewCard.runWriteScript(touchType, scriptData);
	},
	
	// 读取卡片信息
	readCardInfo: function(){
		IcRenewCard.clearAllInput();
		var touchType = FormUtils.getRadioedValue('touchType');
		// 1001 上电
		if(!IcRenewCard.powerOn(touchType)){
			return false;
		}
		// 1003 选择金融环境
		if(!IcRenewCard.onSelect(touchType)) {
			return false;
		}
		// 1004 开始交易
		if(!IcRenewCard.startTrans(touchType)) {
			return false;
		}
		var cardId = IcRenewCard.getCardId(touchType);
		if(isEmpty(cardId)){
			return false;
		}
		// 查询卡的信息
		if(!IcRenewCard.loadCardInfo(cardId)) {
			return false;
		}
		// 1011 取得卡片余额
		if(!IcRenewCard.getCardBanlance(touchType)) {
			return false;
		}
	},
	
	readNewCardId: function(){
		var touchType = FormUtils.getRadioedValue('touchType');
		// 1001 上电
		if(!IcRenewCard.powerOn(touchType)){
			return false;
		...
		$('#id_newCardId').val(cardId);
		// 1002 下电
		var power = IcRenewCard.powerOff(touchType);
		if(power == "-1"){
			return false;
		}
		return true;
	...
	checkForm: function(){
		...
		if(!IcRenewCard.checkEcashDeposit()) {
			return false;
		}
		...
		var authAmt = IcRenewCard.leftPad(newBal[0], 12, '0'); //授权金额为0
		
		// 1017 获得交易密文ARQC
		if(!IcRenewCard.getTransARQC(touchType, authAmt)) {
			return false;
		}
		
		// 1012 获取终端验证结果
		if(!IcRenewCard.getTerminalCheck(touchType)) {
			return false;
		}
		...
		$.post(CONTEXT_PATH + '/pages/icRenewCard/add.do', 
				params, 
				function(data){
					// 获取信息成功的话
					if(data.result) {
						...
						var revesalURL = '/pages/icRenewCard/reversal.do?icCardReversal.refId=' + refId;
						
						// 1020 进行发卡行认证
						var cardAuth = IcRenewCard.getCardBrancAuth(touchType, arpc);
						if(cardAuth == "-1"){
							gotoUrl(revesalURL);
							return false;
						}
						...
						if(chkRespCode == '00' && writeRespCode == '00'){
							// 写卡 
							writeRuslt = IcRenewCard.writeCard(touchType, arpc, authAmt, scriptData);
						}
	...
	CheckUSBKey: function() {
	    ...
	    try{
			...
			if (isOnline == 0) {
				if(IcRenewCard.FTDoSign()){ //调用FT的签名函数
					return true;
				} else {
					return false;
				}
	...
	FTDoSign: function() {
		...
		if (SetCertResultRet == 0) {
			...
			$('#id_serialNo').val(serialNumber);
	   		...
	   		var cardId = $('#id_cardId').val(); // 旧卡卡号
	   		var oldBalanceAmt = $('#id_lastBalance').val(); // 旧卡电子现金余额
			
			// 取得要验签的内容(旧卡卡号 + 新卡卡号)
	        var signResultRet = document.all.FTUSBKEYCTRL.SignResult(cardId +  fix(oldBalanceAmt) + randomNum);
			if (signResultRet == 0) {
