...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<f:js src="/js/biz/verify/verify.js"/>
		...
		<script>
			$(function(){
				...
				$('#id_recvAmt').blur(function(){
					...
					alert(parseFloat(value) == parseFloat(remain));
					if(parseFloat(value) == parseFloat(remain)){
				...
				<table class="form_grid detail_tbl" width="100%" border="0" cellspacing="3" cellpadding="0">
					...
					<tr>
						...
						<td height="30">
							...
							<input type="button" value="返回" name="escape" onclick="gotoUrl('/verify/verifyReleaseCardFee/list.do?goBack=goBack')" class="ml30" />
						</td>
					</tr>
				</table>
				<s:token name="_TOKEN_VERIFY_RELEASE_CARD_FEE_VERIFY"/>
			</s:form>
		</div>
