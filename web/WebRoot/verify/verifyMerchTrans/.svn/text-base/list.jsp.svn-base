	...
	<body>
				...
				<s:form action="list.do" id="searchForm" cssClass="validate-tip">
					<table id="searchForm" class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						...
						<tr>
							...
							<td>
								<s:select name="mset.payCode" headerKey="" headerValue="--请选择--" list="branchList" listKey="branchCode" listValue="branchName"></s:select>
							</td>
							<td width="80" height="30" align="right">商户</td>
							<td>
								<s:select name="mset.recvCode" headerKey="" headerValue="--请选择--" list="merchList" listKey="merchId" listValue="merchName"></s:select>
							</td>
					...
					<s:token name="_TOKEN_VERIFY_MERCH_TRANS_LIST"/>
				</s:form>
			</div>
		...
		<div class="tablebox">
			...
			<tr>
			   ...
			   <td align="center" nowrap class="titlebg">实际核销金额</td>
			...
			<tr>
			  ...
			  <td align="right" nowrap>${fn:amount(feeAmount)}</td>
			  ...
			  <td align="center" nowrap>
			  	<s:if test="chkStatus==01 || chkStatus==04">
				  	<span class="redlink">
				  		<f:pspan pid="verifymerchtrans_verify">
				  			<f:link href="/verify/verifyMerchTrans/showVerify.do?mset.payCode=${payCode}&mset.recvCode=${recvCode}&mset.setDate=${setDate}&mset.transType=${transType}&mset.curCode=${curCode}">核销</f:link>
				  		</f:pspan>
				  	</span>
				</s:if>
			  </td>
