	...
	<body>
		...
		<div class="userbox">
			...
			<s:form action="verify.do" id="inputForm" cssClass="validate">
				<s:hidden name="mset.recvCode" />
				<s:hidden name="mset.payCode" />
				<s:hidden name="mset.setDate" />
				<s:hidden name="mset.curCode" />
				<s:hidden name="mset.transType" />
				...
				<table class="form_grid detail_tbl" width="100%" border="0" cellspacing="3" cellpadding="0">
					...
					<tr>
						...
						<td>
							<s:textfield id="id_recvAmt" name="mset.recvAmt" cssClass="{required:true}"></s:textfield>
					...
					<tr>
						...
						<td height="30">
							...
							<input type="button" value="返回" name="escape" onclick="gotoUrl('/verify/verifyCardBranch/list.do?goBack=goBack')" class="ml30" />
						</td>
					</tr>
				</table>
				<s:token name="_TOKEN_VERIFY_CARD_BRANCH_VERIFY"/>
			</s:form>
		</div>
