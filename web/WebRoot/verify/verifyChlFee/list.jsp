			...
			<div class="contentb">
				<s:form action="list.do" id="searchForm" cssClass="validate-tip">
					<table id="searchForm" class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						<caption>运营中心分支机构平台手续费核销</caption>
						<tr>
							<td align="right">分支机构</td>
							<s:if test="showFenzhi">
							<td>
								<s:select name="chlFeeMSet.chlCode" list="chlList" listKey="branchCode" listValue="branchName"></s:select>
							</td>
							</s:if>
							<s:else>
								<td><s:textfield name="chlFeeMSet.chlName" /></td>
							</s:else>
							<td align="right">核销状态</td>
							<td>
								<s:select name="chlFeeMSet.chkStatus" list="chkStatusList" headerKey="" headerValue="--请选择--" listKey="value" listValue="name"></s:select>
							</td>
					...
					<s:token name="_TOKEN_VERIFY_CHL_FEE_LIST"/>
				</s:form>
			</div>
			...
			<tr>
			  ...
			  <td align="center" nowrap>
			  	<s:if test="chkStatus==01 || chkStatus==04">
				  	<span class="redlink">
				  		<f:pspan pid="verifychlfee_verify">
				  			<f:link href="/verify/verifyChlFee/showVerify.do?chlFeeMSet.chlCode=${chlCode}&chlFeeMSet.feeMonth=${feeMonth}">核销</f:link>
				  		</f:pspan>
				  	</span>
				</s:if>
			  </td>
