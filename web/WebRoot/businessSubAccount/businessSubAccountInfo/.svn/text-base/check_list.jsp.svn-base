	...
	<body>
		...
		<div class="tablebox">
			...
			<tr>
			  ...
			  <td align="center" nowrap>${cashAmount}</td>
			  <td align="center" nowrap>${freezeCashAmount}</td>
			  ...
			  <td align="center" nowrap>
			  	<span class="redlink">
			  		...
			  		<c:if test="${'Y' eq canBind && '00' eq state}">
			  			<f:link href="/businessSubAccount/businessSubAccountInfo/changeState.do?businessSubAccountInfo.accountId=${systemId}&show">实体账户</f:link>
			  		</c:if>
			  		<c:if test="${'Y' eq mySystem}">
			  			<c:if test="${'00' eq state}">
			  				<f:link href="/businessSubAccount/businessSubAccountInfo/changeState.do?businessSubAccountInfo.accountId=${systemId}&accountSystemInfo.state=04">冻结</f:link>
			  			</c:if>
			  			<c:if test="${'04' eq state}">
			  				<f:link href="/businessSubAccount/businessSubAccountInfo/changeState.do?businessSubAccountInfo.accountId=${systemId}&accountSystemInfo.state=00">解冻</f:link>
			  			</c:if>
			  			<c:if test="${'01' eq state}">
			  				<f:link href="/businessSubAccount/businessSubAccountInfo/showCheck.do?businessSubAccountInfo.accountId=${systemId}">审核</f:link>
			  			</c:if>
			  		</c:if>
			  	</span>
			  </td>
