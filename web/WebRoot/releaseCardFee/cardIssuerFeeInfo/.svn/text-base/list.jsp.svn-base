				...
				<s:form id="searchForm" action="list.do" cssClass="validate-tip">
					<table class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						...
						<tr>
							...
							<s:select name="cardIssuerFeeDTotal.chlCode" list="chlList" listKey="branchCode" listValue="branchName"></s:select>
							</s:if>
							<s:else>
							<s:textfield name="cardIssuerFeeDTotal.chlName"></s:textfield>
							</s:else>
							</td>
							<td align="right">发卡机构</td>
							<td>
							<s:if test="showCard">
							<s:select name="cardIssuerFeeDTotal.branchCode" list="branchList" listKey="branchCode" listValue="branchName"></s:select>
							</s:if>
							<s:else>
							<s:textfield name="cardIssuerFeeDTotal.branchName"></s:textfield>
							</s:else>
							</td>
							<td align="right">计费日期</td>
							<td>
						...
						<tr>
							...
							<td>
							<s:select name="cardIssuerFeeDTotal.transType" list="chlFeeContentTypeList" headerKey="" headerValue="--请选择--" listKey="value" listValue="name"></s:select>
							</td>
							<td align="right">计费方式</td>
							<td>
							<s:select name="cardIssuerFeeDTotal.feeType" list="feeTypeList" headerKey="" headerValue="--请选择--" listKey="value" listValue="name"></s:select>
							</td>
						</tr>
						<tr>
							<td></td>	
					...
					<s:token name="_TOKEN_CARDISSUERFEEINFO_LIST"/>
				</s:form>
			</div>
		...
		<div class="tablebox">
			...
			<tr>		
			  ...
			  <td align="center" nowrap>
			  	<span class="redlink">
			  		<f:link href="/releaseCardFee/cardIssuerFeeInfo/detail.do?cardIssuerFeeDTotal.feeDate=${feeDate}&cardIssuerFeeDTotal.chlCode=${chlCode}&cardIssuerFeeDTotal.branchCode=${branchCode}&cardIssuerFeeDTotal.transType=${transType}">查看</f:link>
			  	</span>
			  </td>
			</tr>
			</s:iterator>
