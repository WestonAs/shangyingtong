	...
	<body>
				...
				<s:form id="searchForm" action="list.do" cssClass="validate-tip">
					<table class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						...
						<tr>
							...
							<td>
							<s:if test="showCard">
							<s:select name="releaseCardFeeDTotal.branchCode" headerKey="" headerValue="--请选择--" list="branchList" listKey="branchCode" listValue="branchName"></s:select>
							</s:if>
							<s:else>
							<s:textfield name="releaseCardFeeDTotal.branchName"></s:textfield>
							</s:else>
							</td>
					...
					<s:token name="_TOKEN_RELEASECARDFEEINFO_LIST"/>
				</s:form>
			</div>
		...
		<div class="tablebox">
			...
			<tr>		
			  ...
			  <td align="left" nowrap><s:if test="merchId == 0">通用商户</s:if><s:else>${merchId}-${fn:branch(merchId)}${fn:merch(merchId)}</s:else></td><!-- ${merchId}-${merchName} -->
			  <td align="center" nowrap>${cardBin}</td>
			  <td align="center" nowrap>${transTypeName}</td>
			  <td align="center" nowrap>${feeTypeName}</td>
			  <td align="right" nowrap>
				<s:if test="feeType == 1 || feeType == 2 || feeType == 3">${fn:amount(feeRate)}</s:if><s:else>${fn:percentPre(feeRate, 4)}</s:else>
			  </td>
			  <td align="right" nowrap>${fn:amount(amount)}</td>
			  <td align="right" nowrap>${fn:amount(feeAmt)}</td>
			  <td align="center" nowrap>
			  	<span class="redlink">
			  		<f:link href="/releaseCardFee/releaseCardFeeInfo/detail.do?releaseCardFeeDTotal.feeDate=${feeDate}&releaseCardFeeDTotal.branchCode=${branchCode}&releaseCardFeeDTotal.merchId=${merchId}&releaseCardFeeDTotal.cardBin=${cardBin}&releaseCardFeeDTotal.transType=${transType}&releaseCardFeeDTotal.posManageId=${posManageId}&releaseCardFeeDTotal.posProvId=${posProvId}">查看</f:link>
			  	</span>
			  </td>
			</tr>
			</s:iterator>
