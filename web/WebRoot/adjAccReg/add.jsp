...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script>
		function checkTrans(){
			if (!FormUtils.hasRadio('adjAccReg.transSn')){
				alert('未选择交易记录');
				return false;
			}

			if($("#id_regAmt").val() - $("#enableAdjAmt").text() > 0 ){
				alert("调账金额不能大于原交易的可调账金额");
				return false;
			}
		...
		function checkCardIssuer(){
			if($("#idCardIssuer").length>0 && $("#idCardIssuer").val()!=$("#idLloginBranchCode").val() && $("[name='trans.cardId']").val()==""){
	...
	<body>
		...
		<div class="userbox">
			...
			<div class="contentb">
				<s:form id="searchForm" action="findAdj.do" cssClass="validate-tip">
						...
						<tr>
							...
							<s:if test="loginRoleType == 40 || loginRoleType == 20">
								...
								<td>
									...
									<s:hidden id="idLloginBranchCode" type="hidden" name="loginBranchCode" disabled="disabled"/>
								</td>
							</s:if>
					...
					<s:token name="_TOKEN_ADJACCREG_ADD"/>
				</s:form>
			</div>
		...
		<s:form id="searchForm" action="add.do" cssClass="validate">
			...
			<div class="tablebox">
				...
				<tr>
				   ...
				   <td align="center" nowrap class="titlebg">可调账金额</td>
				</tr>
				</thead>
				<s:iterator value="page.data"> 
				<tr>
				  <td align="center" nowrap>
				  	<input type="radio" name="adjAccReg.transSn" value="${transSn}" 
				  		onclick="$('#id_regAmt').val(fix(${settAmt-goodsAmt}));$('#enableAdjAmt').html(fix(${settAmt-goodsAmt}));"/>
				  </td>
				  ...
				  <td align="right" nowrap>${fn:amount(settAmt-goodsAmt)}</td>
				</tr>
				</s:iterator>
				</table>
			...
			<div class="contentb clear" id="idInputPage">
				<table class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
					<tr>
						<td width="80" height="30" align="right">调账金额</td>
						<td>
							<s:textfield id="id_regAmt" name="adjAccReg.amt" cssClass="{required:true, num:true} "/>
							<span class="field_tipinfo">请输入调账金额</span>
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							可调账金额：
							<span id='enableAdjAmt' class="bigred"></span>
						</td>
					</tr>
					<tr>
