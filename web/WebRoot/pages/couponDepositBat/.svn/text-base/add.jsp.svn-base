...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function calcRefAmt() {
			...
			$('#id_RefAmt').val(point);
			$('#id_DepositCouponSum').val(point*cardCount);
			$('#id_RefAmtSum').val(point*cardCount);
			$('#input_btn2').removeAttr('disabled')
			//$.post(CONTEXT_PATH + '/pages/couponDepositBat/calRealAmt.do', {'couponClass':couponClass, 'cardCount':cardCount, 'point':point, 'callId':callId()},
			//	function(json){
			//		if (json.success){
			//			$('#id_RefAmt').val(fix(json.refAmt));
			//			$('#id_DepositCouponSum').val(json.depositCouponSum);
		...
		function checkCardIds() {
			...
			$.post(CONTEXT_PATH + '/pages/couponDeposit/checkCard.do', {'cardId':cardId, 'cardCount':cardCount, 'callId':callId()},
				function(json){
					if (json.success){
			...
			var couponClass = $('#id_CouponClass').val();
			var point = $('#id_CouponAmt').val();
			if(!isEmpty(couponClass) && !isEmpty(point) && !isEmpty(cardCount)){
				calcRefAmt();
			}
		}
				...
				<s:form action="add.do" id="inputForm" cssClass="validate">
					...
					<table class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						...
						<tr>
							...
							<td>
								<s:textfield id="id_strCardId" name="depositCouponReg.strCardId" maxLength="19" cssClass="{required:true, num:true}"/>
						...
						<tr>
							...
							<td>
								<s:textfield id="id_endCardId" name="depositCouponReg.endCardId" cssClass="{required:true, digit:true} readonly" maxlength="19" readonly="true" />
						...
						<tr>
							<td width="80" height="30" align="right">单张赠送赠券</td>
							<td>
								<s:textfield id="id_CouponAmt" name="depositCouponBatReg.couponAmt" cssClass="{required:true, decimal:'12,2'}"/>
								<span class="field_tipinfo">请输入赠券</span>
							</td>
							<td width="80" height="30" align="right">折算金额</td>
							<td>
								<s:textfield id="id_RefAmt" name="depositCouponBatReg.refAmt" cssClass="{required:true} readonly" readonly="true"/>
						...
						<tr>
							...
							<td>
								<s:textfield id="id_DepositCouponSum" name="depositCouponReg.couponAmt" cssClass="{required:true, decimal:'12,2'} readonly" readonly="true"/>
								<span class="field_tipinfo">请输入赠券</span>
							</td>
					...
					<s:token name="_TOKEN_DEPOSIT_COUPON_BAT_REG_ADD"/>
				</s:form>
