...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<s:if test='externalCardImportReg.externalNumMakeCard'>
			...
			<s:set var="externalImportNotice" value='"开卡上传文件名格式：kk_发卡机构编号_当前日期_批次.txt ，如：kk_10005850_20120828_001.txt"'/>
			<s:set var="tmplFileName" value='"kk_xxxxxxxx_yyyyMMdd_001.txt"'/>
		</s:if>
		<s:else>
			<s:set var="pageTitle" value='"外部卡导入"'/>
			<s:set var="listActionDo" value='"list.do"'/>
			<s:set var="externalImportNotice" value='"上传文件名格式规范：发卡机构编号_当前日期_批次.txt ，如：10005850_20120828_001.txt"'/>
			<s:set var="tmplFileName" value='"xxxxxxxx_yyyyMMdd_001.txt"'/>
		</s:else>
		...
		<script type="text/javascript">
			$(function(){
				Selector.selectCardBin('binName', 'binNo', true, '${loginBranchCode}', '', loadSubClass);
				$("#cardSubclass").change(findPointClassByCardSubclass);
			});
		</script>
	</head>
	...
	<body>
		...
		<div class="userbox">
			...
			<div class="contentb">
				<s:form action="add.do" id="inputForm" enctype="multipart/form-data" cssClass="validate">
					<table class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						...
						<tr>
							...
							<td>
								<s:hidden id="binNo" name="externalCardImportReg.binNo" />
								<s:textfield id="binName" cssClass="{required:true}"/>
						...
						<tr>
							...
							<td>
								<select id="cardSubclass" name="externalCardImportReg.cardSubclass" class="{required:true}"></select>
								<span class="field_tipinfo">请选择卡子类型</span>
							</td>
							<td width="80" height="30" align="right">通用积分类型</td>
							<td>
								<s:hidden id="ptClass" name="externalCardImportReg.ptClass" />
								<s:textfield id="ptClassName" cssClass="readonly" readonly="true" />
						...
						<tr>
							...
							<td >
								<s:textfield name="externalCardImportReg.remark" maxlength="256"/>
				...
				<ul class="showli_div">
					<li class="showli_div">${externalImportNotice }，下载 <b><a href="${tmplFileName }">${pageTitle }模板</a></b></li>
