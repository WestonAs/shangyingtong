...
<html xmlns="http://www.w3.org/1999/xhtml">
		...
		<script>	
			$(function(){
				...
				if ('${SESSION_USER.role.roleType}'=='00'||'${SESSION_USER.role.roleType}'=='02' ){
					Selector.selectBranch('idBranchCode_sel', 'id_payCode', true, '20');
				} else if ('${SESSION_USER.role.roleType}'=='01' ){
					Selector.selectBranch('idBranchCode_sel', 'id_payCode', true, '20', '', '', '${SESSION_USER.branchNo}');
				}
				
				$('#id_reportType').change(changeReportType);
				$('#id_payCode').change(changePayCode);
			});
			
			function changePayCode(){
				var payCode = $('#id_payCode').val();
				$('#merchNames').removeMulitselector();
				Selector.selectMerch('merchNames', 'merchIds', false, payCode); // 发卡机构的特约商户
			}
			
			function changeReportType(){
				var reportType = $('#id_reportType').val();
				if(reportType == '1'){ // 交易清算日结算报表
	...
	<%
	 	ICardReportLoad operateFee = (ICardReportLoad)SpringContext.getService("cardTransDSetImpl");
	...
	<body>
					...
					<table class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
							...
							<c:if test="${showCardList}">
								...
								<c:choose>
									  <c:when test='${SESSION_USER.role.roleType eq "00" || SESSION_USER.role.roleType eq "01" || SESSION_USER.role.roleType eq "02"}'>
											<input type="hidden" id="id_payCode" name="payCode" value="${param.payCode}"/>
											<input type="text" id="idBranchCode_sel" name="branchName" value="${param.branchName}" class="{required:true}"/>
									  </c:when>
									  <c:otherwise>
											<select id="id_payCode" name="payCode" class="{required:true}" >
												<option value="">--请选择--</option>
						...
						<tr>
							...
							<c:if test="${showMerchList}">
								<td id="merch_td1"  align="right" >商户</td>
								<td id="merch_td2" >
	                                    <input id="merchIds" type="hidden" name="merchs"  value="${param.merchs}" />
									    <input id="merchNames" type="text" name="merch_names"  value="${param.merch_names}" />
								</td>
							</c:if>
						</tr>
						<tr>
		...
		<%
			...
			if(showCardList) {
				...
				String cardBranchCode = (String) request.getAttribute("cardBranch");
				params.append("cardBranchCode=" + cardBranchCode + ";");
				BranchInfo fenzhi = (BranchInfo) branchInfoDAO.findByPk(cardBranchCode);
				params.append("cardBranchName=" + fenzhi.getBranchName() + ";");
			}
			...
			if(showMerchList){
			    String merchs =request.getParameter("merchs");
			    if(StringUtils.isNotBlank(merchs)){
			        params.append("merchs=" + merchs + ";");
			        params.append("merchsStr=" + merchs + ";");
			    }
			}
		...
		<report:html name="report1" reportFileName="/card/merchTransRMASum.raq"	
		...
		<report:html name="report2" reportFileName="/card/transDSet.raq"	
		    params="<%=params.toString() %>"
		    funcBarFontFace="宋体"               
			funcBarFontSize="14px"   	
			needSaveAsExcel="yes"
