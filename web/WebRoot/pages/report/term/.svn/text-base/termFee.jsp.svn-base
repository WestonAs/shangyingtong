...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<%
	 	ICardReportLoad operateFee = (ICardReportLoad)SpringContext.getService("provShareImpl");
	...
	<body>
		...
		<div class="userbox">
			...
			<div class="contentb">
				<form id="searchForm" action="termFee.jsp" method="post" class="validate-tip">
					<table class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						<caption>机具出机方运营手续费及分润报表</caption>
						<tr>
							...
							<td align="right" id="id_CardList_td1">机具出机方</td>
							<td id="id_CardList_td2">
								<select id="id_payCode" name="provCode" class="{required:true}" onmouseover="FixWidth(this)">
									<option value="">--请选择--</option>
								<c:forEach items="${cardBranchList}" var="u">
									<option value="${u.branchCode}" <c:if test="${param.provCode eq u.branchCode}">selected</c:if>><c:out value="${u.branchName }"/></option>	
								</c:forEach>
								</select>
							</td>
							</c:if>
		...
		%>
		<% if(StringUtils.isNotBlank(feeMonth)){ %>
			<report:html name="report1" reportFileName="/term/operateFeeProv.raq"	
