...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<%
	 	ICardReportLoad operateFee = (ICardReportLoad)SpringContext.getService("manageShareImpl");
	...
	<body>
		...
		<div class="userbox">
			...
			<div class="contentb">
				<form id="searchForm" action="termRepFee.jsp" method="post" class="validate-tip">
					<table class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						<caption>机具维护方运营手续费及分润报表</caption>
						<tr>
							...
							<c:if test="${showCardList}">
								<td align="right" id="id_CardList_td1">机具维护方</td>
								<td id="id_CardList_td2">
									<select id="id_payCode" name="manageCode" class="{required:true}" onmouseover="FixWidth(this)">
										<option value="">--请选择--</option>
										<c:forEach items="${cardBranchList}" var="u">
											<option value="${u.branchCode}" <c:if test="${param.manageCode eq u.branchCode}">selected</c:if>>
												<c:out value="${u.branchName }"/>
											</option>	
										</c:forEach>
									</select>
								</td>
		...
		%>
		<% if(StringUtils.isNotBlank(feeMonth)){ %>
			<report:html name="report1" reportFileName="/term/operateFeeManage.raq"	
