...
<html xmlns="http://www.w3.org/1999/xhtml">
		...
		<script>
		    ...
		    function showBranch(value){
				if(value == '0'){
					...
					$("#id_shdl_td1").show();
					$("#id_shdl_td2").show();					
					$('#id_shdlList').removeAttr('disabled');
				} else if(value == '2'){
					//显示商户代理名称及其列表其余则隐藏					
					$("#id_cardBranch_td1").hide();
					$("#id_cardBranch_td2").hide();
	...
	<%
	 	ICardReportLoad shfrReportLoad = (ICardReportLoad)SpringContext.getService("cardProxyShareReport");
	...
	<body>
		...
		<div class="userbox">
			...
			<div class="contentb">
				<form id="searchForm" action="proxyShare.jsp" method="post" class="validate-tip">
					<table class="form_grid" width="100%" border="0" cellspacing="3  cellpadding="0">
						<caption>商户代理分润汇总报表</caption>
						<tr>
						...
						<tr>
							...
							<td id="id_shdl_td1" align="right" style="display: none;">商户代理</td>
							<td id="id_shdl_td2" style="display: none;">
								<select id="id_shdlList" name="shdlCode" class="{required:true}" disabled="disabled" onmouseover="FixWidth(this)">
									<option value="">--全部--</option>
								<c:forEach items="${shdlList}" var="t">
									<option value="${t.branchCode}" <c:if test="${param.shdlCode eq t.branchCode}">selected</c:if>><c:out value="${t.branchName }"/></option>	
								</c:forEach>
								</select>
							</td>
							 <td></td>
		...
		<% if(!checkSubmit) { %>
			<%
			    ...
			    String shdlCode = request.getParameter("shdlCode");	
			    ...
			    if(StringUtils.isEmpty(branchInfo)) {
			       ...
			       if(StringUtils.isNotEmpty(branchCode))  {
			         queryParams=  shfrReportLoad.getReportQueryParams(roleType,reportType,new String[] {feeDate,branchCode});
			       } 
			       //当选择商户代理时
			       else if(StringUtils.isNotEmpty(shdlCode)) {
			         queryParams = shfrReportLoad.getReportQueryParams(roleType,reportType,new String[] {feeDate,shdlCode});
			       }		      
			    } 
			    ...
			    //否则是发卡机构或商户代理的选择
			       ...
			       if(StringUtils.isNotEmpty(branchCode)) {
			         queryParams = shfrReportLoad.getReportQueryParams(roleType,reportType,new String[] {feeDate,branchInfo,branchCode});
			       } else if(StringUtils.isNotEmpty(shdlCode)) {
			         queryParams = shfrReportLoad.getReportQueryParams(roleType,reportType,new String[] {feeDate,branchInfo,shdlCode});
			       }		       		      
			    }
			    
			    boolean checkParams = ArrayUtils.isEmpty(queryParams);
			    
			    //System.out.println("queryParams====={" + queryParams[0] + "," + queryParams[1] + "}");
			 %>
			 
			 <% if(!checkParams){ %> 
				 <report:html name="shfrReport" reportFileName="<%=queryParams[0]%>"	
