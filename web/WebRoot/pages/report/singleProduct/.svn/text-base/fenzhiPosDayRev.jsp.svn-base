...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<%
	 	ICardReportLoad operateFee = (ICardReportLoad)SpringContext.getService("singleProductFenzhiPOSDayRev");
	...
	<body>
		...
		<div class="userbox">
			...
			<div class="contentb">
				<form id="searchForm" method="post" action="fenzhiPosDayRev.jsp" class="validate-tip">
					<table class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						<caption>${ACT.name}</caption>
						<tr>
							<td align="right">统计日期</td>
							<td>
								<input id="id_feeMonth" type="text" name="feeMonth" onclick="WdatePicker({dateFmt:'yyyyMMdd'})" class="{required:true}" value="${param.feeMonth}" />
							</td>
		...
		<%
			...
			// 计费日期
			String feeDate = request.getParameter("feeMonth");
