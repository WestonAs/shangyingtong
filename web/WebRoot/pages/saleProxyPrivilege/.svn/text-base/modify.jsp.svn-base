	...
	<head>
		...
		$(function(){
			$('#treebox_tree').html(LOAD_IMAGE);
				$loadTree('/pages/saleProxyPrivilege/initTree.do?cardPrivilegeGroup.id=${cardPrivilegeGroup.id}', 'treebox_tree', true);
		});
		function checkTree(){
			...
			<div class="contentb">
				<s:form action="modify.do" id="inputForm" cssClass="validate">
					...
					<table class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						...
						<tr>
							<td width="80" height="30" align="right">权限名</td>
							<td>
								<s:textfield name="cardPrivilegeGroup.name"/>
								<span class="field_tipinfo">请输入权限名</span>
							</td>
						</tr>
						<tr>
							<td width="100" height="30" align="right" valign="top" style="padding-top: 10px">分配权限</td>
							<td>
						...
						<tr>	
							...
							<td height="30" colspan="3">
								...
								<input type="button" value="返回" name="escape" onclick="gotoUrl('/pages/saleProxyPrivilege/list.do?goBack=goBack')" class="ml30" />
							</td>
						</tr>
					</table>
					<s:token name="_TOKEN_SALE_PROXY_PRIVILEGE_MODIFY"/>
					<s:hidden id="privileges" name="privileges" />
					<s:hidden id="privileges" name="cardPrivilegeGroup.id" />
				</s:form>
			</div>
