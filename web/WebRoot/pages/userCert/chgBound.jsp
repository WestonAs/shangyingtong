...
<html xmlns="http://www.w3.org/1999/xhtml">
		...
		function merchSelected(){
		  ...
		  $(function(){
			    ...
			    $("#id_branchNo").focus(function(){
					...
					//var merchNo = $('#id_merchNo').val();
					$('#id_merchNo').val('');
					loadUserInfoList(branchCode,'');					
				});
				$("#id_merchNo").focus(function(){
				    //var branchCode = $('#id_branchNo').val();
				    $('#id_branchNo').val('');
		  ...
		  function loadUserInfoList(branchCode,merchNo) {
		       ...
		       $('#id_userId').load(CONTEXT_PATH + '/pages/userCert/loadUserInfoList.do', 
						{'callId':callId(), 'branchCode':branch_arr[0],'merchNo' : merchNo_arr[0],'userId':$('#id_userId').val()});
		  }
	...
	<body >
		...
		<div class="userbox">
			...
			<div class="contentb">
				<s:form action="chgBound.do" id="inputForm" cssClass="validate">
					<div>
					<table class="form_grid" width="98%" border="0" cellspacing="2" cellpadding="0">
						<caption>用户数字证书变更绑定</caption>
						<tr>
						...
						<tr>
							<td width="80" height="30" align="right">变更用户列表:</td>
							<td>
							    <select id="id_userId" name="userInfo.userId" cssClass="{required:true}"></select>
							    <span class="field_tipinfo">请选择变更绑定用户</span>
							</td>
						</tr>
