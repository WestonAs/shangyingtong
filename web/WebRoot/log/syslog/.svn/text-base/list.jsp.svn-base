	...
	<body>
				...
				<s:form action="list.do" cssClass="validate-tip">
					<table id="searchForm" class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						...
						<tr>
							...
							<td><s:select name="sysLog.logType" list="logTypeList" headerKey="" headerValue="--请选择--" listKey="value" listValue="name"></s:select></td>
							
							<td align="right">日志查看状态</td>
							<td><s:select name="sysLog.state" list="viewList" headerKey="" headerValue="--请选择--" listKey="value" listValue="name"></s:select></td>
		...
		<div class="tablebox">
			...
			<tr>
			   ...
			   <td align="center" nowrap class="titlebg">查看状态</td>
			...
			<tr>
			  ...
			  <td nowrap><s:date name="logDate" format="yyyy-MM-dd HH:mm:ss" /></td>
			  <td align="left" nowrap>${content}</td>
			  <td align="center" nowrap>${stateName}</td>
			  <td align="center" nowrap>
			  	<span class="redlink">
			  		<f:link href="/log/syslog/detail.do?sysLog.id=${id}">查看</f:link>
			  	</span>
			</tr>
			</s:iterator>
			</table>
