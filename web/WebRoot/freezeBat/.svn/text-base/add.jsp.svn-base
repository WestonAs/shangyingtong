...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function checkCardIds() {
			...
			$.post(CONTEXT_PATH + '/freezeBat/checkCardId.do', {'cardId':cardId, 'cardCount':cardCount, 'callId':callId()},
				function(json){
					if (json.success){
						$('#id_endCardId').val(json.endCardId);
						$('#idSubAcctType').load(CONTEXT_PATH + '/freeze/getSubAcctList.do',{'freezeReg.cardId':cardId, 'callId':callId()});
	...
	<body>
				...
				<s:form action="add.do" id="inputForm" cssClass="validate">
					...
					<table class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						...
						<tr>
							...
							<td>
							<s:textfield id="id_strCardId" name="freezeReg.startCard" onkeyup="this.value=this.value.replace(/\D/g,'')" 
							...
							<td>
								<s:textfield id="id_CardCount" name="freezeReg.cardNum" cssClass="{required:true, digit:true}"/>
						...
						<tr>
							...
							<td>
							<s:textfield id="id_endCardId" name="freezeReg.endCard" cssClass="{required:true, digit:true} readonly" maxlength="19" readonly="true" />
								<span class="field_tipinfo" >请输入卡号</span>
							</td>
						</tr>
						<tr>
							...
							<td>
							<select name="freezeReg.subacctType" id="idSubAcctType"	 class="{required:true}"></select>
						...
						<tr>
							...
							<td height="30" colspan="3">
								...
								<input type="button" value="返回" name="escape" onclick="gotoUrl('/freezeBat/list.do?goBack=goBack')" class="ml30" />
							</td>
						</tr>
					</table>
					<s:token name="_TOKEN_BAT_FREEZE_ADD"/>
				</s:form>
			</div>
		...
		<div class="userbox">
			...
			<div class="contentb">
				...
				<li class="showli_div">批量冻结的卡必须为操作机构发行的卡或者所售的卡</li>
				<li class="showli_div">起始卡号和结束卡号位数要一致，卡BIN也需一致。</li>
				<li class="showli_div">起始卡号和结束卡号之间不能超过1000张。</li>
				<li class="showli_div">新增冻结金额是指每张卡冻结的金额，如一批次卡中有卡不足余额冻结的，整批次处理失败。</li>
				</ul>
			</div>
			<b class="b4"></b><b class="b3"></b><b class="b2"></b><b class="b1"></b>
		</div>
