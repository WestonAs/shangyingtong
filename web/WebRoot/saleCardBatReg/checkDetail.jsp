	...
	<body>
		...
		<table class="detail_grid" width="98%" border="1" cellspacing="0" cellpadding="1">
			<caption>批量<s:text name="actionSubLabel"></s:text><s:text name="cardTypeCode.cardTypeName"></s:text>信息
				<span class="caption_title"> | <f:link href="/saleCardBatReg/checkList.do?goBack=goBack">返回列表</f:link></span></caption>
		  	<tr>
				<td>售卡批次ID</td>
			...
			<tr>
		  	...
		  	<tr>
		  		<td>售卡机构</td>
				<td>${fn:branch(saleCardReg.branchCode)}${fn:dept(saleCardReg.branchCode)}</td>
				<td>激活机构</td>
				<td>${fn:branch(saleCardReg.activateBranch)}${fn:dept(saleCardReg.activateBranch)}</td>
				<td>售卡撤销标志</td>
		  	...
		  	<tr>
				...
				<td>${saleCardReg.saleDate}</td>
		  		<td>发卡机构号</td>
		  		<td>${saleCardReg.cardBranch}</td>
		  		<td>发卡机构名称</td>
		  		<td>${fn:branch(saleCardReg.cardBranch)}</td>
		  	</tr>
		  	<tr>
		  		<td>付款户名</td>
		  	...
		  	<tr>
				...
				<td>${saleCardReg.updateUser}</td>
		  		<td>更新时间</td>
				<td colspan="3"><s:date name="saleCardReg.updateTime" format="yyyy-MM-dd HH:mm:ss" /></td>
		  	</tr>
		</table>
		</div>
		...
		<!-- 批量售卡登记簿明细 -->
		<s:if test="saleCardBatPage.data.size() > 0">
		<div class="tablebox">
			<b><s:label>批量售卡登记簿明细 </s:label></b>
			<form id="id_detail_bat" method="post" action="checkDetail.do?saleCardBatReg.saleBatchId=${saleCardBatReg.saleBatchId}">
			<table class="data_grid" width="100%" border="0" cellspacing="0" cellpadding="0">			
			<thead>
			<tr>
