		...
		<script>
			$(function(){
				...
				$('#id_CardBranch').change(function(){
					var rebateCard = $('#idRebateCard').val();
					if(!isEmpty(rebateCard)){
						checkRebateCard();
					}
				});
				$('#idRebateCard').change(function(){
					checkRebateCard();
				});
			...
			function checkRebateCard(){
				$('#idRebateCard_field').html('请输入返利卡号');
				var value = $('#idRebateCard').val();
				...
				<s:form action="add.do" id="inputForm" cssClass="validate">
					...
					<table class="form_grid" width="100%" border="0" cellspacing="3" cellpadding="0">
						...
						<tr>
							...
							<td>
								<s:select id="id_CardBranch" name="cardCustomer.cardBranch" list="cardBranchList" headerKey="" headerValue="--请选择--" listKey="branchCode" listValue="branchName" cssClass="{required:true}"></s:select>
						...
						<tr>	
							...
							<td id="idRebateCard_td2" style="display: none;">
								...
								<span class="error_tipinfo">卡号为19位数字</span>
							</td>
						</tr>						
						<tr>
						...
						<tr>
							...
							<td height="30" colspan="3">
								...
								<input type="button" value="返回" name="escape" onclick="gotoUrl('/customerRebateMgr/cardCustomer/list.do?goBack=goBack')" class="ml30" />
							</td>
						</tr>
					</table>
					<s:token name="_TOKEN_CARDCUSTOMER_ADD"/>
				</s:form>
			</div>
